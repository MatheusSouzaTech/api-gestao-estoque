...
module.exports = app => {
    ...
    app.post("/saidas", (req, res) => {
        ...
        app.db.query(`SELECT quantidade FROM Products WHERE _id=?`, [produto_id], (err, result => {
            ...
            if (result[0] < 10) {
                ...
                app.db.query("INSERT INTO ProductExits (_id,produto_id,quantidade,tipoSaida,cliente_id,numeroPedidoVenda,dataSaida,observacoes,lote_numeroLote) VALUES (?,?,?,?,?,?,?,?,?)",
                    [_id,
                        ...
                        dataSaida,
                        observacoes,
                        lote_numeroLote], (err2) => {
                            if (err2) {
                                console.error("Erro ao registrar Saida!", err)
                                return res.status(500).json({ mensagem: "Erro no Servidor!" })
                            }
                            app.db.query("UPDATE Products SET quantidadeEstoque = quantidadeEstoque - ? WHERE _id=?", [quantidade, produto_id], (err3, result3) => {
                                if (err3) {
